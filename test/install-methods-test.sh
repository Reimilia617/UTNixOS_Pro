...
#     （$0=bash、本地没有 install.sh 文件）→ install.sh 自包含执行；
#     需要配置源码时（安装/修复）脚本自己从 GIT_URL 拉取完整仓库再继续。
#
#   方式 B — git clone 到本地后安装
#     `git clone <repo> /tmp/methodB && bash /tmp/methodB/install.sh install`
#     install.sh 就在克隆目录里（$0 存在且目录完整）→ 直接复用本地仓库。
#
...
# 固定为中文，使下方针对中文输出的断言保持稳定（双语实现在 install.sh 内嵌字典）
...
echo "============================================================"
# 方式 A 模拟 curl | bash：只有 install.sh 单文件（旁边没有完整仓库），
# 触发「自包含拉取」——安装需要配置源码时，脚本自己从 GIT_URL 拉取完整仓库。
rm -rf /tmp/alone && mkdir -p /tmp/alone && cp /repo/install.sh /tmp/alone/install.sh
rm -f /tmp/stub.log; rm -rf /mnt/etc; mkdir -p /mnt/etc
INPUT=()   # 全部默认 + 确认安装（新菜单顺序：引导→主题→桌面→…→确认）
pty_in "bash /tmp/alone/install.sh install" /tmp/mA.out \
  '' '' '' '' '' '' '' '' '' '' '' 'y'   # 菜单9回车 + 用户名/密码默认各回车 + 确认
echo "--- 输出片段 ---"
grep -E "正在从 GitHub 获取|复用已拉取的代码|安装完成|✗|错误" /tmp/mA.out | head -10
grep -q "正在从 GitHub 获取最新 UTNixOS_Pro 代码" /tmp/mA.out \
  && ok "A: curl|bash 自包含执行，安装时自动拉取仓库" || bad "A: 未自动拉取代码"
...
[ -f /mnt/etc/nixos/install.sh ] && ok "A: 单文件 install.sh 已随配置部署" || bad "A: install.sh 未部署"
...
check "A: 内置 webui 导入启用" '^[[:space:]]*\./modules/system/webui\.nix' "$CFG"
check "A: 内置 ut 命令导入启用" '^[[:space:]]*\./modules/system/ut\.nix' "$CFG"
...
INPUT=()   # 全部默认 + 确认安装
pty_in "bash /tmp/methodB/install.sh install" /tmp/mB.out \
  '' '' '' '' '' '' '' '' '' '' '' 'y'   # 菜单9回车 + 用户名/密码默认各回车 + 确认
echo "--- 输出片段 ---"
grep -E "复用已拉取的代码|安装完成|✗|错误" /tmp/mB.out | head -10
grep -q "复用已拉取的代码" /tmp/mB.out \
  && ok "B: 使用克隆目录内的完整仓库（未触发拉取）" || bad "B: 走了拉取分支"
...
# 克隆目录保留 .git，验证 Web 面板「更新配置」可复用 git 源（方式 B 的完整生命周期）
[ -d /mnt/etc/nixos/.git ] && ok "B: 部署到 /mnt 时保留了 .git（供 Web 面板更新配置）" || bad "B: .git 未保留"
...
mkdir -p /tmp/fakeprof /run/current-system/bin
...
rm -f /tmp/stub.log
INPUT=()   # 回滚到 generation 22（单文件自包含，不依赖网络）
...
grep -q 'UT紧急回滚' /tmp/mC.out && ok "curl|bash --rollback 显示 UT紧急回滚" || bad "缺少 UT紧急回滚 标题"
