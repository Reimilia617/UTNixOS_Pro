...
declare -A L10N_EN=(
  ...
  [dash_opt7]="  7) One-click repair (pull latest code, rebuild /etc/nixos)"
  [dash_opt8]="  8) Exit"
  [dash_choose]="Choose [1-8]: "
  [auto_repair_hint]="Detected that /etc/nixos is missing/broken (flake.nix not found); entering one-click repair..."
  ...
  [upd_incomplete]="Warning: config incomplete after git reset (flake.nix/install.sh missing); check /etc/nixos before rebuilding"
  [upd_dl_incomplete]="Downloaded config is incomplete (missing flake.nix); keeping the current config, nothing was replaced"
  [upd_swap_fail]="Failed to swap the config directory; the previous config was restored"
...
declare -A L10N_ZH=(
  ...
  [dash_opt7]="  7) 一键修复（拉取最新代码重建 /etc/nixos）"
  [dash_opt8]="  8) 退出"
  [dash_choose]="请选择 [1-8]: "
  [auto_repair_hint]="检测到 /etc/nixos 配置缺失/损坏（缺 flake.nix），进入一键修复..."
  ...
  [upd_incomplete]="警告：git reset 后配置不完整（缺 flake.nix/install.sh），重建前请检查 /etc/nixos"
  [upd_dl_incomplete]="下载的配置不完整（缺 flake.nix），已保留现有配置，未做任何替换"
  [upd_swap_fail]="替换配置目录失败，已回滚到原配置"
