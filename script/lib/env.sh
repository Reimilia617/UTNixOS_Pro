# UTNixOS 脚本配置（script/lib/env.sh）
# 被 script/main.sh 自动加载。修改这里的配置会影响所有命令。

# ---------- 仓库与路径 ----------
# 支持环境变量覆盖（fork 或本地测试）：UTNIXOS_GIT_URL / UTNIXOS_TARBALL_URL / UTNIXOS_RAW_URL
...
RAW_URL="${UTNIXOS_RAW_URL:-https://raw.githubusercontent.com/Reimilia617/UTNixOS/main/install.sh}"   # curl|bash 用的单文件地址
INSTALL_DIR="/etc/nixos"          # 已安装系统上的配置目录
MOUNT_ROOT="/mnt"                 # live 环境挂载点
HOSTNAME="reimilia"               # 主机名（默认）
STATE_FILE=".utnixos-selection"   # 选择状态文件（相对配置目录）
