...
# ---------- 选择状态持久化 ----------
# STATE_VERSION：状态文件格式版本。v2 = webui 默认启用之后的格式；
# 旧状态文件（无版本号）重放时会把默认启用的 webui 注释掉，需要迁移。
save_state() {
...
load_state() {
  ...
  if [[ -f "$st" ]]; then
    ...
    local k v line state_version=""
    while IFS= read -r line; do
      [[ "$line" == \#* || -z "$line" ]] && continue
      k="${line%%=*}"
      v="${line#*=}"
      case "$k" in
        STATE_VERSION)   state_version="$v" ;;
        DESKTOP)         DESKTOP="$v" ;;
        BOOT)            BOOT="$v" ;;
    ...
    GRUB_THEME="${GRUB_THEME:-no}"
    GRUB_DEVICE="${GRUB_DEVICE:-}"
    # 状态迁移：v1（webui 默认开启之前的格式）里没有 webui，
    # 重放时补上，避免旧状态把默认启用的 webui 注释掉（8090 永远起不来）
    if [[ "$state_version" != "2" && " $SYSTEM_MODULES " != *" webui "* ]]; then
      SYSTEM_MODULES="${SYSTEM_MODULES:+$SYSTEM_MODULES }webui"
    fi
    ok state_restored
  fi
}
